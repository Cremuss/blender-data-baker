...
And here's the documented unpacking algorithm:
...
> [!NOTE]
> This may seem complex, but the packing is only applied during the baking process, offline, and the unpacking done in the vertex shader involves just a couple of bitwise operations—the fastest operations. The cost is well buried under the cost of the GPU having to wait for the dependent texture fetches anyway, more on that below.
